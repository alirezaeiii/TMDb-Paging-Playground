...
class PageKeyedTVShowsDataSource(private val dataSource: MoviesRemoteDataSource,
                                 ...
                                 private val query: String) : PageKeyedItemDataSource<TVShow, MovieApi.TVShowWrapper>() {

    override fun getItems(response: Response<MovieApi.TVShowWrapper>): List<TVShow> {
        val data = response.body()?.tvShows
        return data?.map { it } ?: emptyList()
    }

    override fun fetchItems(page: Int): Call<MovieApi.TVShowWrapper> {
        if (sortType != null) {
