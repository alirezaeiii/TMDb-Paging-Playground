...
class TestUpcomingTVSeries : BaseMainActivity() {
    ...
    @Rule
    @JvmField
    val activityTestRule: ActivityTestRule<OnTheAirTVShowActivity> =
        object : ActivityTestRule<OnTheAirTVShowActivity>(
            OnTheAirTVShowActivity::class.java
        ) {
            override fun getActivityIntent(): Intent {
                val targetContext: Context =
                    InstrumentationRegistry.getInstrumentation().targetContext
                return Intent(targetContext, OnTheAirTVShowActivity::class.java)
            }
        }
